...
exports.modules = {
  /***/ 4578: /***/ (
    ...
    var types_snake = __webpack_require__(777); // CONCATENATED MODULE: ../svg-creator/xml-utils.ts
    const h = (element, attributes) =>
      `<${element} ${toAttribute(attributes)}/>`;
    const toAttribute = (o) =>
      Object.entries(o)
        .filter(([, value]) => value !== null)
        .map(([name, value]) => `${name}="${value}"`)
        .join(" "); // CONCATENATED MODULE: ../svg-creator/css-utils.ts
     ...
     */
    const createAnimation = (name, keyframes) =>
      `@keyframes ${name}{` +
    ...
    const minifyCss = (css) =>
      css
        ...
        .replace(/[,;:{}()]\s+./g, (a) => a.replace(/\s+/g, ""))
        .replace(/\;\s*\}/g, "}")
        .trim(); // CONCATENATED MODULE: ../svg-creator/snake.ts

    const lerp = (k, a, b) => (1 - k) * a + k * b;
    const createSnake = (chain, { sizeCell, sizeDot }, duration) => {
      const snakeN = chain[0]
        ? (0, types_snake /* getSnakeLength */.T$)(chain[0])
        : 0;
    ...
    const removeInterpolatedPositions = (arr) =>
      arr.filter((u, i, arr) => {
    ...
    const createGrid = (
      cells,
      { sizeDotBorderRadius, sizeDot, sizeCell },
      duration,
    ) => {
      const svgElements = [];
      const styles = [
    ...
    const getCellsFromGrid = ({ width, height }) =>
      Array.from({ length: width }, (_, x) =>
        Array.from({ length: height }, (_, y) => ({ x, y })),
      ).flat();
    const createLivingCells = (grid0, chain, cells) => {
      const livingCells = (cells ?? getCellsFromGrid(grid0)).map(
        ({ x, y }) => ({
          x,
          y,
          t: null,
