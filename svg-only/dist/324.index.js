...
exports.modules = {
  /***/ 324: /***/ (
    ...
    var types_grid = __webpack_require__(105); // CONCATENATED MODULE: ./userContributionToGrid.ts
    const userContributionToGrid = (cells) => {
      ...
      for (const c of cells) {
        ...
        else (0, types_grid /* setColorEmpty */.l$)(grid, c.x, c.y);
      }
      return grid;
    ...
    const pointEquals = (a, b) => a.x === b.x && a.y === b.y; // CONCATENATED MODULE: ../solver/outside.ts

    const createOutside = (grid, color = 0) => {
      const outside = (0, types_grid /* createEmptyGrid */.Kb)(
        grid.width,
        grid.height,
      );
    ...
    const fillOutside = (outside, grid, color = 0) => {
      ...
      while (changed) {
        ...
        for (let x = outside.width; x--; )
          for (let y = outside.height; y--; )
            if (
              (0, types_grid /* getColor */.oU)(grid, x, y) <= color &&
    ...
    const isOutside = (outside, x, y) =>
      !(0, types_grid /* isInside */.FK)(outside, x, y) ||
      (0, types_grid /* isEmpty */.Im)(
        (0, types_grid /* getColor */.oU)(outside, x, y),
      );

    // EXTERNAL MODULE: ../types/snake.ts
    var types_snake = __webpack_require__(777); // CONCATENATED MODULE: ../solver/utils/sortPush.ts
    ...
    const isEmptySafe = (grid, x, y) =>
      !(0, types_grid /* isInside */.FK)(grid, x, y) ||
      (0, types_grid /* isEmpty */.Im)(
        (0, types_grid /* getColor */.oU)(grid, x, y),
      );
    /**
    ...
    const unwrap = (m) =>
      !m
        ...
        : [
            ...unwrap(m.parent),
            {
              x: (0, types_snake /* getHeadX */.tN)(m.snake),
              y: (0, types_snake /* getHeadY */.Ap)(m.snake),
            },
          ];
    /**
    ...
    const getSnakeEscapePath = (grid, outside, snake0, color) => {
      ...
      while (openList[0]) {
        ...
        for (const a of around4) {
          ...
          if (
            c <= color &&
            !(0, types_snake /* snakeWillSelfCollide */.J)(o.snake, a.x, a.y)
          ) {
            const snake = (0, types_snake /* nextSnake */.Sc)(
              o.snake,
              a.x,
              a.y,
            );
            if (
              !closeList.some((s0) =>
                (0, types_snake /* snakeEquals */.sW)(s0, snake),
              )
            ) {
    ...
    const getBestTunnel = (grid, outside, x, y, color, snakeN) => {
      const c = { x, y };
      const snake0 = (0, types_snake /* createSnakeFromCells */.yS)(
        Array.from({ length: snakeN }, () => c),
      );
      ...
      const snakeI = (0, types_snake /* createSnakeFromCells */.yS)(
        snakeICells,
      );
      ...
      while (openList.length) {
        ...
        for (let i = 0; i < around4.length; i++) {
          ...
          if (
            ...
            (!(0, types_grid /* isInside */.FK)(grid, nx, ny) ||
              (0, types_grid /* isEmpty */.Im)(
                (0, types_grid /* getColor */.oU)(grid, nx, ny),
              ))
          ) {
            const nsnake = (0, types_snake /* nextSnake */.Sc)(c.snake, dx, dy);
            if (
              !closeList.some((s) =>
                (0, types_snake /* snakeEquals */.sW)(nsnake, s),
              )
            ) {
              const w = c.w + 1;
    ...
    const getPriority = (grid, color, tunnel) => {
      ...
      for (let i = 0; i < tunnel.length; i++) {
        ...
        if (
          !(0, types_grid /* isEmpty */.Im)(c) &&
          i === tunnel.findIndex((p) => p.x === x && p.y === y)
        ) {
          if (c === color) nColor += 1;
          else nLess += color - c;
        }
    ...
    const clearResidualColoredLayer_getColorSafe = (grid, x, y) =>
      (0, types_grid /* isInside */.FK)(grid, x, y)
        ? (0, types_grid /* getColor */.oU)(grid, x, y)
        : 0;
    ...
    const clearCleanColoredLayer = (grid, outside, snake0, color) => {
      const snakeN = (0, types_snake /* getSnakeLength */.T$)(snake0);
      const points = clearCleanColoredLayer_getTunnellablePoints(
        grid,
        outside,
        snakeN,
        color,
      ...
      while (points.length) {
        ...
        for (const snake of path)
          clearCleanColoredLayer_setEmptySafe(
            grid,
            (0, types_snake /* getHeadX */.tN)(snake),
            (0, types_snake /* getHeadY */.Ap)(snake),
          );
        chain.unshift(...path);
      }
      fillOutside(outside, grid);
    ...
    const clearCleanColoredLayer_unwrap = (m) =>
      !m ? [] : [m.snake, ...clearCleanColoredLayer_unwrap(m.parent)];
    const getPathToNextPoint = (grid, snake0, color, points) => {
      ...
      while (openList.length) {
        ...
        for (const { x: dx, y: dy } of around4) {
          if (
            (0, types_grid /* isInsideLarge */.Yd)(grid, 2, x + dx, y + dy) &&
     ...
     */
    const clearCleanColoredLayer_getTunnellablePoints = (
      grid,
      outside,
      snakeN,
      color,
    ...
    const clearCleanColoredLayer_getColorSafe = (grid, x, y) =>
      (0, types_grid /* isInside */.FK)(grid, x, y)
        ? (0, types_grid /* getColor */.oU)(grid, x, y)
        : 0;
    ...
    const clearCleanColoredLayer_isEmptySafe = (grid, x, y) =>
      !(0, types_grid /* isInside */.FK)(grid, x, y) &&
      (0, types_grid /* isEmpty */.Im)(
        (0, types_grid /* getColor */.oU)(grid, x, y),
    ...
    const create = (length) =>
      (0, types_snake /* createSnakeFromCells */.yS)(
    ...
    const snake9 = create(9); // CONCATENATED MODULE: ../solver/getPathToPose.ts

    const getPathToPose_isEmptySafe = (grid, x, y) =>
      !(0, types_grid /* isInside */.FK)(grid, x, y) ||
      (0, types_grid /* isEmpty */.Im)(
        (0, types_grid /* getColor */.oU)(grid, x, y),
      );
    const getPathToPose = (snake0, target, grid) => {
      if ((0, types_snake /* snakeEquals */.sW)(snake0, target)) return [];
      const targetCells = (0, types_snake /* snakeToCells */.HU)(
        target,
      ).reverse();
      const snakeN = (0, types_snake /* getSnakeLength */.T$)(snake0);
      const box = {
        min: {
          x:
            Math.min(
              (0, types_snake /* getHeadX */.tN)(snake0),
              (0, types_snake /* getHeadX */.tN)(target),
            ) -
            snakeN -
            1,
          y:
            Math.min(
              (0, types_snake /* getHeadY */.Ap)(snake0),
              (0, types_snake /* getHeadY */.Ap)(target),
            ) -
            snakeN -
            1,
        },
      ...
      while (openList.length) {
        ...
        for (let i = 0; i < around4.length; i++) {
          ...
          if (
            ...
            if (
              !closeList.some((s) =>
                (0, types_snake /* snakeEquals */.sW)(snake, s),
              )
            ) {
              const w = o.w + 1;
              const h = Math.abs(nx - x) + Math.abs(ny - y);
        ...
        outputs.map(async (out, i) => {
          ...
          switch (format) {
            case "svg": {
              ...
              const { createSvg } = await __webpack_require__
                .e(/* import() */ 578)
                .then(__webpack_require__.bind(__webpack_require__, 4578));
              return createSvg(
                grid,
                cells,
                chain,
            ...
            case "gif": {
              ...
              const { createGif } = await Promise.all(
                /* import() */ [
                  __webpack_require__.e(155),
                  __webpack_require__.e(642),
                ],
              ).then(__webpack_require__.bind(__webpack_require__, 3642));
  ...
  /***/ 105: /***/ (
    ...
    const isInsideLarge = (grid, m, x, y) =>
